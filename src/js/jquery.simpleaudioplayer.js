...
$.fn.simpleAudioPlayer = function (options) {
	...
	function SiriWave(t) {
		(t = t || {}),
			...
			(this.curves = []);
		for (var i = 0; i < SiriWaveCurve.prototype.definition.length; i++)
			this.curves.push(
				new SiriWaveCurve({
					controller: this,
	...
	(SiriWaveCurve.prototype._globAttenuationEquation = function (t) {
		return (
			null == SiriWaveCurve.prototype._globAttenuationEquation.cache[t] &&
			(SiriWaveCurve.prototype._globAttenuationEquation.cache[t] = Math.pow(
				4 / (4 + Math.pow(t, 4)),
				4
			)),
			SiriWaveCurve.prototype._globAttenuationEquation.cache[t]
		);
	}),
		(SiriWaveCurve.prototype._globAttenuationEquation.cache = {}),
		(SiriWaveCurve.prototype._xpos = function (t) {
			return this.controller.cache.width2 + t * this.controller.cache.width4;
		}),
		(SiriWaveCurve.prototype._ypos = function (t) {
			var i =
		...
		(SiriWave.prototype._interpolate = function (t) {
		...
		(SiriWave.prototype.setAmplitude = function (t) {
	...
	!(function () {
		var t = function () {
			(this.audioAdapter = t._getAdapter(this)),
				(this.events = {}),
				(this.sections = []),
				this.bind("update", e);
		};
		function e() {
			for (var t in this.sections)
				this.sections[t].condition() && this.sections[t].callback.call(this);
		}
		(t.version = "0.3.2"),
			(t.adapters = {}),
			(t.prototype = {
					...
					return (
						e instanceof HTMLElement
							...
							: ((this.source = window.Audio ? new Audio() : {}),
								(this.source.src = t._makeSupportedPath(e.src, e.codecs))),
		...
		(function (t) {
			...
			(t.options = {}),
				(t.setOptions = function (e) {
					for (var i in e) e.hasOwnProperty(i) && (t.options[i] = e[i]);
				}),
				(t.isSupported = function () {
					return window.Float32Array && window.Uint32Array
						? ((t = !!(navigator.vendor || "").match(/Apple/)),
							(e = (e = navigator.userAgent.match(/Version\/([^ ]*)/))
								? parseFloat(e[1])
								: 0),
							(t && e <= 6) ||
				...
				(t.canPlay = function (t) {
					i.canPlayType;
					return !(
						!i.canPlayType ||
						!i.canPlayType(e[t.toLowerCase()]).replace(/no/, "")
					);
				}),
				(t.addPlugin = function (e, i) {
					void 0 === t.prototype[e] && (t.prototype[e] = i);
				}),
				(t._makeSupportedPath = function (e, i) {
					if (!i) return e;
					for (var n = 0; n < i.length; n++)
						if (t.canPlay(i[n])) return e + "." + i[n];
					return e;
				}),
				(t._getAdapter = function (e) {
					switch (t.isSupported()) {
						case "webaudio":
							return new t.adapters.webaudio(e);
						default:
							return null;
					}
				}),
				(t._getMP3SrcFromAudio = function (t) {
					var e = t.children;
					if (t.src) return t.src;
					for (var i = e.length; i--;)
						if ((e[i].type || "").match(/audio\/mpeg/)) return e[i].src;
					return null;
				});
		})(window.Dancer),
		(function () {
			...
			(e.prototype = {
				load: function (e) {
					...
					return (
						...
						this.audio.addEventListener("progress", function (t) {
							t.currentTarget.duration &&
								(n.progress =
									t.currentTarget.seekable.end(0) / t.currentTarget.duration);
						}),
						this.audio
					);
				},
				...
				update: function (e) {
					if (this.isPlaying && this.isLoaded) {
						var i,
							n = [],
							o = e.inputBuffer.numberOfChannels,
							r = t / o,
							s = function (t, e) {
								return t[i] + e[i];
							};
						for (i = o; i--;) n.push(e.inputBuffer.getChannelData(i));
						for (i = 0; i < r; i++)
							this.signal[i] = o > 1 ? n.reduce(s) / o : n[0][i];
					}
				},
			}),
				(Dancer.adapters.webaudio = e);
		})();
	...
	return this.each(function (index, element) {
		const audio = new Dancer();
		// some setup
		var self = $(this);
		var hash = $(classes.player).length;
		if (settings.chapters.length < 0) return;

		if (self.prop("tagName").toLowerCase() != "audio") {
			return false;
		}

		settings.title = settings.chapters[0].title;

		var audioSrc = settings.chapters[0].src;
		var obj_audio = new Audio(audioSrc);

		audio.load(obj_audio);
		...
		var wrapper =
			'<div class="' +
			classes["player"].substr(1) +
			'">' +
			'<div class="containerTop cf">' +
			'<div class="' +
			classes["progressBar"].substr(1) +
			'">' +
			'<div class="' +
			classes["progressBarInner"].substr(1) +
			'"></div>' +
			'<div class="' +
			classes["progressIndicator"].substr(1) +
			'"></div>' +
			"</div>" +
			'<div class="trackInfo cf">' +
			'<div class="' +
			classes["progressTime"].substr(1) +
			'">&nbsp;</div>' +
			'<div class="trackTitle">' +
			settings.title +
			"</div>" +
			'<div class="' +
			classes["durationTime"].substr(1) +
			'">&nbsp;</div>' +
			"</div>" +
			'<div class="controls row">' +
			'<div class="col c2 ' +
			classes["menuToggle"].substr(1) +
			'">&nbsp;</div>' +
			'<div class="col c2 ' +
			classes["backward"].substr(1) +
			'"><span class="icon icon-backward"></span></div>' +
			'<div class="col c4 ' +
			classes["play"].substr(1) +
			'"><span class="icon icon-play"></span></div>' +
			'<div class="col c4 hide ' +
			classes["pause"].substr(1) +
			'"><span class="icon icon-pause"></span></div>' +
			'<div class="col c2 ' +
			classes["forward"].substr(1) +
			'"><span class="icon icon-forward"></span></div>' +
			'<div class="col c2 ' +
			classes["stop"].substr(1) +
			'"><span class="icon icon-stop"></span></div>' +
			"</div>" +
			"</div>" +
			'<div class="containerBottom cf">' +
			'<div class="' +
			classes["chapterList"].substr(1) +
			'">' +
			"</div>" +
			"</div>" +
			"</div>";
		...
		if (settings.chapters.length > 0) {
			...
			for (var i = 0; i < settings.chapters.length; i++) {
				chaptersList +=
					'<li><a id="audio_chap' +
					settings.chapters[i].chap +
					'" class="' +
					classes["chapterLink"].substr(1) +
					(i == 0 ? " active" : "") +
					'" data-chapter="' +
					settings.chapters[i].chap +
					'"  data-link="' +
					settings.chapters[i].src +
					'" >' +
					settings.chapters[i].title +
					"</a></li>";
			}
			chaptersList += "</ul>";
		...
		get("chapterLink").on("click", setUrl);
		get("progressBar")
		...
		function setUrl() {
			...
			self.attr("src", link);
			self.attr("data-chap", $(this).attr("data-chapter"));
			if (self.prop("tagName").toLowerCase() != "audio") {
				return false;
			}
			$(".chapterLink").removeClass("active");
			$(this).addClass("active");
			$("#a1").trigger("reload");
			obj_audio = new Audio(link);
			audio.load(obj_audio);
			sapPlay();
			get("pause").removeClass("hide");
			get("play").addClass("hide");
			refreshUI();
		}
		...
		function fadeOut(duration) {
			duration =
				typeof duration !== "undefined" ? duration : settings.fadeOutSpeed;
		...
		function offsetIsValid(offset) {
			var positiveValidOffset =
				offset > 0 && audio.audio.currentTime < audio.audio.duration - offset;
			var negativeValidOffset =
				offset < 0 && audio.audio.currentTime + offset > 0;
